...
def load_state(file_path):
    try:
        ...
        state = {"loss_count": 0, "consecutive_wins": 0}  # Initialize missing keys
    return state
...
async def allinone(signal_data):
    ...
    adjusted_time = target_time - datetime.timedelta(seconds=15)

    while True:
        current_time = datetime.datetime.now()   #wait for 10 seconds b4 target time 
